...
def insert_fixture(id, home_team, away_team, fixture_date, venue_name, venue_city, referee, status):
    ...
    try:
        ...
        print(f"Erro ao inserir fixture {id}: {e}")
    finally:
        if conn:
            conn.close()
...
def get_all_fixtures():
    ...
    try:
        ...
        return [{'id': row[0], 'home_team': row[1], 'away_team': row[2], 'fixture_date': row[3], 'venue_name': row[4], 'venue_city': row[5], 'referee': row[6], 'status': row[7]} for row in fixtures]
    except Exception as e:
