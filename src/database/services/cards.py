...
def get_cards(fixture_id):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT player_name, team_name, card_type FROM cards WHERE fixture_id = %s", (fixture_id,))
...
def insert_card(fixture_id, player_name, team_name, card_type):
    if not record_exists("SELECT 1 FROM cards WHERE fixture_id = %s AND player_name = %s AND card_type = %s", (fixture_id, player_name, card_type)):
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('''
